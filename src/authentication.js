  ...
  return {
    ...
    startServer: function (callback, port) {
      ...
      app.get(
        '/auth/callback',
        function (req, res) {
          this.getTokenRequest(req.query.code)
            .then(
              function (data) {
        ...
        function (resolve, reject) {
          if (type == 'oauth') {
            ...
            if (this._tokens.user.Token) {
              var user_expire_user = new Date(
                this._tokens.user.NotAfter
              ).getTime();
              ...
              if (new Date() > user_expire_user) {
                ...
                this.refreshToken(this._tokens.oauth.refresh_token)
                  .then(
                    function (user_token) {
                      Debug(
                        'refreshTokens(' + type + ') Get token,',
                        user_token
                      );
                      ...
                      this.getUserToken(this._tokens.oauth.access_token)
                        .then(
                          function (token) {
                        ...
                        .catch(function (error) {
                          reject(
                            'Unable to refresh oauth access token. Reauthenticate again'
                          );
                        });
                    }.bind(this)
                  )
                  .catch(function (error) {
                    reject(
                      'Unable to refresh user access token. Reauthenticate again'
                    );
                  });
              } else {
            ...
            if (this._tokens.xsts.Token) {
              ...
              if (new Date() > oauth_expire) {
                ...
                this.getXstsToken(this._tokens.user.Token)
                  .then(
                    function (token) {
                      ...
                      this.refreshTokens('xsts')
                        .then(function () {
                          Debug(
                            'refreshTokens(' + type + ') Token xsts flow ok'
                          );
                          resolve();
                        })
                        .catch(function (error) {
                          Debug(
                            'refreshTokens(' + type + ') Token xsts flow failed'
                          );
                          reject(error);
                        });
                    }.bind(this)
                  ...
                  .catch(
                    function (error) {
                      ...
                      Debug(
                        'refreshTokens(' + type + ') Token refresh failed:',
                        error
                      );

                      reject(
                        'Unable to refresh xsts access token. Reauthenticate again',
                        error
                      );
                    }.bind(this)
                  );
    ...
    getTokenRequest: function (code, callback) {
      return new Promise(
        function (resolve, reject) {
          ...
          HttpClient()
            .post(
              this._endpoints.live + '/oauth20_token.srf',
    ...
    getUserToken: function (accessToken) {
      ...
      return new Promise(
        function (resolve, reject) {
          ...
          HttpClient()
            .post(
              this._endpoints.auth + '/user/authenticate',
    ...
    getXstsToken: function (userToken) {
      ...
      return new Promise(
        function (resolve, reject) {
          ...
          HttpClient()
            .post(
              this._endpoints.xsts + '/xsts/authorize',
              {
                'Content-Type': 'application/json',
